...
@DisplayName("AuthController (paciente) - Pruebas unitarias")
class AuthControllerTest {
    ...
    void login_retorna200() throws Exception {
        AuthResponse response = new AuthResponse(1L, "jwt-paciente-token", "PACIENTE", "Juan Perez", "/paciente");
        ...
        mockMvc.perform(post("/api/v1/auth/login")
                ...
                .andExpect(jsonPath("$.token").value("jwt-paciente-token"))
                .andExpect(jsonPath("$.rol").value("PACIENTE"))
                .andExpect(jsonPath("$.nombre").value("Juan Perez"));
    }

    @Test
    @DisplayName("POST /register → 201 CREATED con token asignado")
    void register_retorna201() throws Exception {
        AuthResponse response = new AuthResponse(3L, "register-token", "PACIENTE", "Pedro Ramirez", "/paciente");
        when(authService.registrar(any(RegisterRequest.class))).thenReturn(response);

        RegisterRequest req = new RegisterRequest("pedro@mail.com", "pass789", "Pedro Ramirez");
        ...
        mockMvc.perform(post("/api/v1/auth/register")
                ...
                .andExpect(jsonPath("$.nombre").value("Pedro Ramirez"))
